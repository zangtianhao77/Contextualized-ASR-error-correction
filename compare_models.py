...
"""load an ASR model"""
model1 = SpeechRecognitionModel("facebook/wav2vec2-large-960h")
model2 = SpeechRecognitionModel("jonatasgrosman/wav2vec2-large-xlsr-53-english")
model4 = SpeechRecognitionModel("facebook/hubert-large-ls960-ft")
...
def addNoise(speech,display=False):
    ...
    sound1 = AudioSegment.from_mp3(final_path) + 10
...
"""Speech-To-Text"""
def STT_model1(speech):

    # audio_paths = ["/path/to/file.mp3", "/path/to/another_file.wav"]
    audio_paths = [speech]
    # facebook large
    transcriptions = model1.transcribe(audio_paths)

    return transcriptions[0]['transcription']

def STT_model2(speech):

    # audio_paths = ["/path/to/file.mp3", "/path/to/another_file.wav"]
    audio_paths = [speech]
    #jon large
    transcriptions = model2.transcribe(audio_paths)
...
def STT_model4(speech):

    # audio_paths = ["/path/to/file.mp3", "/path/to/another_file.wav"]
    audio_paths = [speech]
    # facebook large
    transcriptions = model4.transcribe(audio_paths)

    return transcriptions[0]['transcription']

def STT_model5(speech):
...
if __name__ == "__main__":
    ...
    for _ in inputs:
        ...
        # compare 3 transcriptions from 3 models
        transcription1=STT_model1(mixed_speech)
        transcription2=STT_model2(mixed_speech)
        transcription3=STT_model3(mixed_speech)
        transcription4=STT_model4(mixed_speech)
        transcription5=STT_model5(mixed_speech)

        #model1 result
        avg_WER_list1 = []
        print("Reference:\t\t"+reference)
        print("Transcription1:\t"+transcription1)
        wer1=WER(reference, transcription1)
        errors1=[]
        if wer1['Ins']>0:
            errors1.append("Insertion")
        if wer1['Del']>0:
            errors1.append("Deletion")
        if wer1['Sub']>0:
            errors1.append("Substitution")
        wer_score1 = WER(reference, transcription1)['WER']
        print("WER1: " + str(wer_score1))
        avg_WER_list1.append(wer_score1)
        print("PER1: " + str(WER(PronunciationDictionary().generate_phone_sequence(reference), PronunciationDictionary().generate_phone_sequence(transcription1))['WER']))
        print(errors1)
        results_json1.append({
            'errors':errors1,
            'reference': reference,
        ...
        wer2=WER(reference, transcription2)
        errors2=[]
        if wer2['Ins']>0:
            errors2.append("Insertion")
        if wer2['Del']>0:
            errors2.append("Deletion")
        if wer2['Sub']>0:
            errors2.append("Substitution")
        wer_score2 = WER(reference, transcription2)['WER']
        print("WER2: " + str(wer_score2))
        avg_WER_list2.append(wer_score2)
        print("PER2: " + str(WER(PronunciationDictionary().generate_phone_sequence(reference), PronunciationDictionary().generate_phone_sequence(transcription2))['WER']))
        print(errors2)
        results_json2.append({
            'errors':errors2,
            'reference': reference,
        ...
        avg_WER_list3 = []
        print("Reference:\t\t"+reference)
        print("Transcription3:\t"+transcription3)
        wer3=WER(reference, transcription3)
        errors3=[]
        if wer3['Ins']>0:
            errors3.append("Insertion")
        if wer3['Del']>0:
            errors3.append("Deletion")
        if wer3['Sub']>0:
            errors3.append("Substitution")
        wer_score3 = WER(reference, transcription3)['WER']
        print("WER3: " + str(wer_score3))
        avg_WER_list3.append(wer_score3)
        print("PER3: " + str(WER(PronunciationDictionary().generate_phone_sequence(reference), PronunciationDictionary().generate_phone_sequence(transcription3))['WER']))
        print(errors3)
        results_json3.append({
            'errors':errors3,
            'reference': reference,
        ...
        avg_WER_list4 = []
        print("Reference:\t\t"+reference)
        print("Transcription4:\t"+transcription4)
        wer4=WER(reference, transcription4)
        errors4=[]
        if wer4['Ins']>0:
            errors4.append("Insertion")
        if wer4['Del']>0:
            errors4.append("Deletion")
        if wer4['Sub']>0:
            errors4.append("Substitution")
        wer_score4 = WER(reference, transcription4)['WER']
        print("WER4: " + str(wer_score4))
        avg_WER_list4.append(wer_score4)
        print("PER4: " + str(WER(PronunciationDictionary().generate_phone_sequence(reference), PronunciationDictionary().generate_phone_sequence(transcription4))['WER']))
        print(errors4)
        results_json4.append({
            'errors':errors4,
            'reference': reference,
            'transcription': transcription4})

        #model5 result
        avg_WER_list5 = []
        print("Reference:\t\t"+reference)
        print("Transcription5:\t"+transcription5)
        wer5=WER(reference, transcription5)
        errors5=[]
        if wer5['Ins']>0:
            errors5.append("Insertion")
        if wer5['Del']>0:
            errors5.append("Deletion")
        if wer5['Sub']>0:
            errors5.append("Substitution")
        wer_score5 = WER(reference, transcription5)['WER']
        print("WER5: " + str(wer_score5))
        avg_WER_list5.append(wer_score5)
        print("PER5: " + str(WER(PronunciationDictionary().generate_phone_sequence(reference), PronunciationDictionary().generate_phone_sequence(transcription5))['WER']))
        print(errors5)
        results_json5.append({
            'errors':errors5,
            'reference': reference,
            'transcription': transcription5})
    ...
    print("AVG_WER_model5_Google_STT:" + str(sum(avg_WER_list5) / (float)(len(avg_WER_list5))))

    """write as json files"""
    with open('output/WikiHow-facebook_large2.json', 'w') as f:
        f.write(json.dumps(results_json1, indent=4))
    
    with open('output/WikiHow-jon_large2.json', 'w') as f:
        f.write(json.dumps(results_json2, indent=4))

    with open('output/WikiHow-pytorch2.json', 'w') as f:
        f.write(json.dumps(results_json3, indent=4))

    with open('output/WikiHow-HuBERT2.json', 'w') as f:
        f.write(json.dumps(results_json4, indent=4))

    with open('output/WikiHow-GoogleSTT2.json', 'w') as f:
        f.write(json.dumps(results_json5, indent=4))

    # with open ('output/WikiHow-facebook_large.json', 'r') as load_f:
    #     input1 = json.load(load_f)

    # with open ('output/WikiHow-jon_large.json', 'r') as load_f:
    #     input2 = json.load(load_f)

    # with open ('output/WikiHow-pytorch.json', 'r') as load_f:

...
#     prosody_tags=[
#         "pitch='medium' rate='slow' volume='soft'",
#         "pitch='medium' rate='medium' volume='soft'",
#         "pitch='low' rate='slow' volume='medium'",
#         "pitch='medium' rate='slow' volume='medium'",
#         "pitch='high' rate='slow' volume='medium'",
#         "pitch='medium' rate='medium' volume='medium'",
#         "pitch='low' rate='slow' volume='loud'",
#         "pitch='medium' rate='slow' volume='loud'",
#         "pitch='high' rate='slow' volume='loud'",
#         "pitch='low' rate='medium' volume='loud'",
#         "pitch='medium' rate='medium' volume='loud'",
#         "pitch='high' rate='medium' volume='loud'"
#     ]
#     tags = random.sample(prosody_tags,1)[0]
#     result = "<speak><prosody "+ tags
#
#     input_text=input_text.replace("&","and").replace("<","less than").replace(">","greater than").replace("'","").replace('"',"")
#     result= result +">"+ input_text +"</prosody></speak>"
#     if display:
#         print(result)
#     return result
#

def testSSML(input_text,display=False):
    output=[]
    prosody_tags={
...
"""Text-TO-Speech"""
def TTS(input_text,VoiceId,path,display=False):
...
if __name__ == "__main__":
    ...
    # ssml=[]
    # for _ in inputs:
    #     ssml.append([_,testSSML(_)])
    # for i in range(len(ssml)):
    #     reference=ssml[i][0]
    #     VoiceId = random.sample(speakers.keys(), 1)[0]
    #     print("Reference:\t\t"+reference)
    #     if not os.path.exists("reference"):
    #         os.makedirs("reference")
    #     reference_speech = TTS("<speak>"+reference+"</speak>", VoiceId, "reference/" + str(i) + ".mp3", True)
    #     transcriptions=[]
    #     for j in ssml[i][1]:
    #         input_text = j[1]
    #         save_path="audio_display/"+j[0]["volume"]+" "+j[0]["rate"]+" "+j[0]["pitch"]
    #         if not os.path.exists(save_path):
    #             os.makedirs(save_path)
    #         speech = TTS(input_text,VoiceId,save_path+"/"+str(i)+".mp3",True)
    #
    #         y1, sr1 = librosa.load(reference_speech)
    #         y2, sr2 = librosa.load(speech)
    ...
    #             'WER':wer["WER"]
    #         })
    #     results_json.append({'reference': str(i)+" - "+reference, 'transcriptions': transcriptions})
    ...
    for i in input:
        ...
        transcriptions = []
        length = len(PronunciationDictionary().generate_phone_sequence(reference).split())-1
        for j in i["transcriptions"]:
            transcriptions.append({
                            "prosody":j["prosody"],
                            "distance":j["distance"],
                            "distance/len": j["distance"] /length,
    ...
    for i in input:
        for j in i["transcriptions"]:
            if str(j["prosody"]) in combine:
                combine[str(j["prosody"])].append(j["distance/len"])
            else:
                combine[str(j["prosody"])]=[]
            for k in j["prosody"]:
                if k+":"+j["prosody"][k] in value:
                    value[k+":"+j["prosody"][k]].append(j["distance/len"])
                else:
                    value[k+":"+j["prosody"][k]] = []
    for _ in combine:
